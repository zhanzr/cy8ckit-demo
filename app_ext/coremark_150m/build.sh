#!/usr/bin/env bash
# coremark_150m - app_ext CM4 CoreMark (external NOR XIP @ 150 MHz).
# Run from this directory in Git Bash / MSYS2:  ./build.sh
#
# Outputs build/cm4_coremark_150m.elf/.hex (0x18000000) + build/cm0p_xip_stub.hex.
...
INC=("-I." "-Icm4/bench" "-Icm4/coremark_1_0_1" "-I$COMMON" "-I$COMMON/ext" "-I$DEVICES/include" "-I$DEVICES/include/ip" "-I$PDL/drivers/include")
...
for f in "cm4/main.c" "cm4/bench/utils.c" "cm4/bench/core_portme.c" \
         "cm4/coremark_1_0_1/core_main.c" "cm4/coremark_1_0_1/core_list_join.c" \
         "cm4/coremark_1_0_1/core_matrix.c" "cm4/coremark_1_0_1/core_state.c" \
         "cm4/coremark_1_0_1/core_util.c" "$COMMON/uart.c" "$COMMON/system_psoc6_cm4.c"; do
  base=$(basename "$f" .c)
...
"$CC" -mcpu=cortex-m4 -mthumb -mfloat-abi=soft -T "$COMMON/cm4.ld" -Wl,--gc-sections -Wl,-Map="$BUILD/cm4.map" \
  "${OBJS[@]}" -lm -lc -lnosys -o "$BUILD/cm4_coremark_150m.elf"

"$OBJCOPY" -O ihex "$BUILD/cm4_coremark_150m.elf" "$BUILD/cm4_coremark_150m.hex"

( cd "$STUB" && bash build.sh )
cp "$STUB/build/cm0p_xip_stub.hex" "$BUILD/"

echo "=== built $BUILD/cm4_coremark_150m.hex (0x18000000) + $BUILD/cm0p_xip_stub.hex ==="
