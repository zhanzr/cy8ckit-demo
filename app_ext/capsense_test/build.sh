#!/usr/bin/env bash
# capsense_test (app_ext) - CAPSENSE buttons + slider demo on CY8CKIT-062-BLE,
...
#
# Outputs:
#   build/cm4_capsense_test.elf / .hex  - link at 0x18000000 (program to the
...
CAPSENSE="$MS/capsense/release-v10.0.0"
...
DEFINES=("-DCY8C6347BZI_BLD53" "-DCOMPONENT_PSOC6_01" "-DCOMPONENT_CAT1A" "-DCY_IPC_DEFAULT_CFG_DISABLE" "-DCOMPONENT_MW_CAPSENSE" "-DCY_USING_HAL")
...
# App + common + generated device config (cycfg_system.c is kept for the
# generated symbols; main() deliberately does not call init_cycfg_system).
for f in "cm4/main.c" "cm4/led.c" "$COMMON/uart.c" "$COMMON/system_psoc6_cm4.c" \
         "cycfg.c" "cycfg_capsense.c" "cycfg_clocks.c" "cycfg_peripherals.c" \
...
# CAPSENSE middleware sources
for f in "$CAPSENSE"/cy_capsense_*.c; do
  base=$(basename "$f" .c)
  "$CC" "${CFLAGS[@]}" -c "$f" -o "$BUILD/cap_$base.o"
  OBJS+=("$BUILD/cap_$base.o")
done

# PDL drivers (capsense + hal usage)
for f in cy_gpio cy_csd cy_scb_common cy_scb_ezi2c cy_scb_i2c cy_scb_uart cy_scb_spi cy_sysclk cy_sysint cy_syslib cy_syspm cy_syspm_bus cy_systick cy_wdt cy_trigmux cy_tcpwm cy_tcpwm_counter cy_tcpwm_pwm; do
...
# Prebuilt CAPSENSE DSP/algorithm routines (the _Lib functions referenced by
# the middleware .c sources)
CAPSENSE_PREBUILT="$CAPSENSE/COMPONENT_SOFTFP/TOOLCHAIN_GCC_ARM/libcy_capsense.a"

"$CC" -mcpu=cortex-m4 -mthumb -mfloat-abi=soft -T "$COMMON/cm4.ld" -Wl,--gc-sections -Wl,-Map="$BUILD/cm4.map" \
  "${OBJS[@]}" "$CAPSENSE_PREBUILT" -lm -lc -lnosys -o "$BUILD/cm4_capsense_test.elf"

"$OBJCOPY" -O ihex "$BUILD/cm4_capsense_test.elf" "$BUILD/cm4_capsense_test.hex"

( cd "$STUB" && bash build.sh )
cp "$STUB/build/cm0p_xip_stub.hex" "$BUILD/"

echo "=== built $BUILD/cm4_capsense_test.hex (0x18000000) + $BUILD/cm0p_xip_stub.hex ==="
