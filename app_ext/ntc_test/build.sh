#!/usr/bin/env bash
# ntc_test (app_ext) - NTC thermistor + DTS temperature demo on CY8CKIT-062-BLE
# + CY8CKIT-028-EPD, CM4 running from the external S25FL512S NOR via SMIF XIP
# (0x18000000).
# Port of app/ntc_test to the app_ext XIP flow.
#
# Run from this directory in Git Bash / MSYS2:  ./build.sh
#
# Outputs:
#   build/cm4_ntc_test.elf / .hex  - link at 0x18000000 (program to the
...
for f in "cm4/main.c" "cm4/ntc.c" "$COMMON/uart.c" "$COMMON/system_psoc6_cm4.c" \
...
# PDL drivers (adc / sar / hal usage)
for f in cy_dma cy_gpio cy_sar cy_sysanalog cy_scb_common cy_scb_uart cy_scb_spi cy_sysclk cy_sysint cy_syslib cy_syspm cy_syspm_bus cy_systick cy_wdt cy_trigmux cy_tcpwm cy_tcpwm_counter cy_tcpwm_pwm; do
...
"$CC" -mcpu=cortex-m4 -mthumb -mfloat-abi=soft -T "$COMMON/cm4.ld" -Wl,--gc-sections -Wl,-Map="$BUILD/cm4.map" \
  "${OBJS[@]}" -lm -lc -lnosys -o "$BUILD/cm4_ntc_test.elf"

"$OBJCOPY" -O ihex "$BUILD/cm4_ntc_test.elf" "$BUILD/cm4_ntc_test.hex"

( cd "$STUB" && bash build.sh )
cp "$STUB/build/cm0p_xip_stub.hex" "$BUILD/"

echo "=== built $BUILD/cm4_ntc_test.hex (0x18000000) + $BUILD/cm0p_xip_stub.hex ==="
