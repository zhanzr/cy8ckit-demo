...
#define CY_CAPSENSE_INPUT_ARRAY_SIZE                    (5u)
#define CY_CAPSENSE_INPUT_ARRAY_SIZE_HALF               (CY_CAPSENSE_INPUT_ARRAY_SIZE / 2)
...
#if (CY_CAPSENSE_SLIDER_CENTROID_5X_EN)
    static void Cy_Capsense_SliderPeakDetect(
                    cy_capsense_advanced_slider_t * ptrAdvSlider,
                    const cy_stc_capsense_widget_config_t * ptrWdConfig);
    static void Cy_Capsense_SliderDetectTouchZone(
                    cy_capsense_advanced_slider_t * ptrAdvSlider,
                    const cy_stc_capsense_widget_config_t * ptrWdConfig);
    static uint16_t Cy_Capsense_SliderCalculatePositionNormalFinger(
                    uint8_t idxPeak,
                    cy_capsense_advanced_slider_t * ptrAdvSlider,
                    const cy_stc_capsense_widget_config_t * ptrWdConfig);
    static uint16_t Cy_Capsense_SliderCalculatePositionFatFinger(
                    uint8_t idxPeak,
                    cy_capsense_advanced_slider_t * ptrAdvSlider,
                    const cy_stc_capsense_widget_config_t * ptrWdConfig);
#endif
...
****************************************************************************//**
*
* Finds the touch position of a Linear slider widget and classifies the touch
* object type (a normal finger, fat finger, or large object).
*
* The function performs the following steps:
* 1. Detects peak sensors whose diff exceeds the finger threshold.
* 2. Determines the touch zone around each peak by expanding left and right
*    while neighboring sensor signals satisfy the signal ratio condition.
* 3. Compares the touch zone width against fat finger and large object
*    size thresholds (adjusted by hysteresis based on the previous finger
*    type) to classify the touch as a normal finger, fat finger, or large
*    object.
* 4. Calculates the touch position using a three-sensor centroid for a normal
*    finger or a weighted centroid across the entire touch zone for a fat finger.
*
* This function does not detect two or more touches.
* 
* \param newTouch
...
{
    #if (CY_CAPSENSE_SLIDER_CENTROID_5X_EN)
        /* Improved slider centroid algorithm variables. */
        cy_capsense_advanced_slider_t advSlider = {0};
        uint16_t touchZoneWidth;
        uint8_t idxPeak;
        uint16_t pos = 0u;
        uint16_t fatFingerTh;
        uint16_t largeObjTh;
        uint32_t lastFingerType = (ptrWdConfig->ptrWdContext->status & CY_CAPSENSE_WD_FINGER_TYPE_MASK) >> CY_CAPSENSE_WD_FINGER_TYPE_POS;

    #else
        /* Legacy three-sensor centroid algorithm variables. */
    ...
    #if (CY_CAPSENSE_SLIDER_CENTROID_5X_EN)
        /* Improved slider centroid algorithm. */
        if (0u != (ptrWdConfig->centroidConfig & CY_CAPSENSE_CENTROID_5X5_MASK))
        {
            Cy_Capsense_SliderPeakDetect(&advSlider, ptrWdConfig);
            Cy_Capsense_SliderDetectTouchZone(&advSlider, ptrWdConfig);

            /* Copy thresholds to local variables to avoid accumulating subtraction */
            fatFingerTh = ptrWdConfig->ptrWdContext->minFatFingerSize;
            largeObjTh  = ptrWdConfig->ptrWdContext->minLargeObjectSize;

            /* Apply hysteresis only when previous finger type is known (not first touch) */
            if (lastFingerType != CY_CAPSENSE_FINGER_TYPE_NONE)
            {
                /* Fat finger threshold: lower only if previously fat (stay fat), raise if normal */
                if (lastFingerType == CY_CAPSENSE_FINGER_TYPE_FAT)
                {
                    fatFingerTh -= ptrWdConfig->minFatFingerSizeHyst;
                }
                else if (lastFingerType == CY_CAPSENSE_FINGER_TYPE_NORMAL)
                {
                    fatFingerTh += ptrWdConfig->minFatFingerSizeHyst;
                }
                else
                {
                    /* Fat threshold stays at base (neutral transition down) */
                }

                /* Large object threshold: lower if previously large (stay), raise otherwise */
                if (lastFingerType == CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT)
                {
                    largeObjTh -= ptrWdConfig->minLargeObjSizeHyst;
                }
                else
                {
                    largeObjTh += ptrWdConfig->minLargeObjSizeHyst;
            ...
            /* Clear finger type bits before determining touch object type. */
            ptrWdConfig->ptrWdContext->status &= (uint32_t)(~(uint32_t)CY_CAPSENSE_WD_FINGER_TYPE_MASK);

            /* Clear position */
            newTouch->numPosition = CY_CAPSENSE_POSITION_NONE;

            /* Once large object is detected, latch it until no-touch clears the status */
            if (lastFingerType == CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT)
            {
                ptrWdConfig->ptrWdContext->status |= ((uint32_t)CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT << CY_CAPSENSE_WD_FINGER_TYPE_POS);
            }
            else
            {
                /* Determine touch object type. */
                for (idxPeak = advSlider.peakNum; idxPeak-- != 0u;)
                {
                    touchZoneWidth = advSlider.lastZoneIdx[idxPeak] - advSlider.firstZoneIdx[idxPeak] + 1u;
                    
                    /* Check if touch is large object. */
                    if (touchZoneWidth >= largeObjTh)
                    {
                        if (0u == (ptrWdConfig->centroidConfig & CY_CAPSENSE_FINGER_TYPE_DETECTION_MASK))
                        {
                            /* Feature is disabled, report finger type normal and calculate position as fat finger */
                            ptrWdConfig->ptrWdContext->status |= ((uint32_t)CY_CAPSENSE_FINGER_TYPE_NORMAL << CY_CAPSENSE_WD_FINGER_TYPE_POS);
                            pos = Cy_Capsense_SliderCalculatePositionFatFinger(idxPeak, &advSlider, ptrWdConfig);
                        }
                        else
                        {
                            ptrWdConfig->ptrWdContext->status |= ((uint32_t)CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT << CY_CAPSENSE_WD_FINGER_TYPE_POS);
                    ...
                    /* Check if touch is fat finger. */
                    else if (touchZoneWidth >= fatFingerTh)
                    {
                        pos = Cy_Capsense_SliderCalculatePositionFatFinger(idxPeak, &advSlider, ptrWdConfig);
                        
                        if (0u == (ptrWdConfig->centroidConfig & CY_CAPSENSE_FINGER_TYPE_DETECTION_MASK))
                        {
                            /* Feature is disabled, report finger type normal */
                            ptrWdConfig->ptrWdContext->status |= ((uint32_t)CY_CAPSENSE_FINGER_TYPE_NORMAL << CY_CAPSENSE_WD_FINGER_TYPE_POS);
                        }
                        else
                        {
                            ptrWdConfig->ptrWdContext->status |= ((uint32_t)CY_CAPSENSE_FINGER_TYPE_FAT << CY_CAPSENSE_WD_FINGER_TYPE_POS);
                        }
                        newTouch->numPosition = CY_CAPSENSE_POSITION_ONE;
                    }
                    /* Touch is normal finger. */
                    else
                    {
                        ptrWdConfig->ptrWdContext->status |= ((uint32_t)CY_CAPSENSE_FINGER_TYPE_NORMAL << CY_CAPSENSE_WD_FINGER_TYPE_POS);
                        pos = Cy_Capsense_SliderCalculatePositionNormalFinger(idxPeak, &advSlider, ptrWdConfig);
                        newTouch->numPosition = CY_CAPSENSE_POSITION_ONE;
                    }

                    newTouch->ptrPosition[0u].x = pos;
                }
            }
        }
    #else
        /* Legacy three-sensor centroid algorithm. */
                ...
                const cy_stc_capsense_widget_config_t * ptrWdConfig)
{
    ...
    {
        ptrWdConfig->ptrWdContext->status &= (uint32_t)~(uint32_t)CY_CAPSENSE_WD_ACTIVE_MASK;
    }
    else
    {
        ptrWdConfig->ptrWdContext->status |= (uint32_t)CY_CAPSENSE_WD_ACTIVE_MASK;
    }
}
#endif /* (CY_CAPSENSE_DISABLE != CY_CAPSENSE_CSX_TOUCHPAD_EN) */
...
#if (CY_CAPSENSE_SLIDER_CENTROID_5X_EN)
/*******************************************************************************
* Function Name: Cy_Capsense_SliderPeakDetect
****************************************************************************//**
*
* This function searches the diff counts array input to detect peaks based on provided thresholds.
*
* \param ptrAdvSlider 
* The pointer to the advanced slider structure.
*
* \param ptrWdConfig 
* The pointer to the widget configuration structure
* \ref cy_stc_capsense_widget_config_t.
...
{
    uint16_t snsIndex;
    uint16_t snsCount = ptrWdConfig->numSns;
    ptrAdvSlider->peakNum = 0u;
    uint32_t touchTh;

    /* Calculate touch threshold based on current widget state */
    touchTh = (0u == (ptrWdConfig->ptrWdContext->status & CY_CAPSENSE_WD_ACTIVE_MASK)) ?
                ((uint32_t)ptrWdConfig->ptrWdContext->fingerTh + ptrWdConfig->ptrWdContext->hysteresis) :
                ((uint32_t)ptrWdConfig->ptrWdContext->fingerTh - ptrWdConfig->ptrWdContext->hysteresis);

    /* Calculate the number of local maxima and find their indexes. */
    /* A < B >= C */
    for (snsIndex = 0u; snsIndex < snsCount; snsIndex++)
    {
        if (ptrWdConfig->ptrSnsContext[snsIndex].diff > touchTh)
        {
            if (snsIndex == 0u)
            {
                if (ptrWdConfig->ptrSnsContext[snsIndex].diff >= ptrWdConfig->ptrSnsContext[snsIndex + 1u].diff)
                {
                    ptrAdvSlider->peakIdx[ptrAdvSlider->peakNum] = snsIndex;
                    ptrAdvSlider->peakNum++;
                }
            }
            else
            {
                if (snsIndex == (snsCount - 1u))
                {
                    if (ptrWdConfig->ptrSnsContext[snsIndex - 1u].diff < ptrWdConfig->ptrSnsContext[snsIndex].diff)
                    {
                        ptrAdvSlider->peakIdx[ptrAdvSlider->peakNum] = snsIndex;
                        ptrAdvSlider->peakNum++;
                    }
                ...
                {
                    if ((ptrWdConfig->ptrSnsContext[snsIndex - 1u].diff < ptrWdConfig->ptrSnsContext[snsIndex].diff) &&
                        (ptrWdConfig->ptrSnsContext[snsIndex].diff >= ptrWdConfig->ptrSnsContext[snsIndex + 1u].diff))
                    {
                        ptrAdvSlider->peakIdx[ptrAdvSlider->peakNum] = snsIndex;
                        ptrAdvSlider->peakNum++;
                    }
...
/*******************************************************************************
* Function Name: Cy_Capsense_SliderDetectTouchZone
****************************************************************************//**
*
* This function searches the diff counts array input to detect peaks based on provided thresholds.
*
                ...
                const cy_stc_capsense_widget_config_t * ptrWdConfig)
{
    ...
    uint16_t idxPeak;
    uint16_t idxPeak1;
    uint8_t peakNumLocal;
    uint8_t temp;
    uint8_t strongestPeakIdx = 0u;
    uint16_t strongestPeakDiff;

    /* Delete false peaks */
    for (idxPeak = 0; idxPeak < ptrAdvSlider->peakNum; idxPeak++)
    {
        /* Check if peak exists */
        if (ptrAdvSlider->peakIdx[idxPeak] != 0xFFFFu)
        {
            /* Process for the left side of the touch zone. */
            if (ptrAdvSlider->peakIdx[idxPeak] != 0u)
            {
                for (snsIndex = ptrAdvSlider->peakIdx[idxPeak]; snsIndex-- != 0u;)
                {
                    if ((256u * ptrWdConfig->ptrSnsContext[snsIndex].diff) > (ptrWdConfig->ptrWdContext->fingerSigThMult * ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx[idxPeak]].diff))
                    {
                        if (idxPeak > 0u)
                        {
                            for (idxPeak1 = idxPeak; idxPeak1-- != 0u;)
                            {
                                if (snsIndex == (uint8_t)ptrAdvSlider->peakIdx[idxPeak1])
                                {
                                    ptrAdvSlider->peakIdx[idxPeak1] = 0xFFFFu;
                                }
                            }
                        }
                    }
            ...
            /* Process for the right side of the touch zone. */
            if ((ptrAdvSlider->peakIdx[idxPeak] + 1u) < snsCount)
            {
                for (snsIndex = (ptrAdvSlider->peakIdx[idxPeak] + 1u); snsIndex < snsCount; snsIndex++)
                {
                    if ((256u * ptrWdConfig->ptrSnsContext[snsIndex].diff) > (ptrWdConfig->ptrWdContext->fingerSigThMult * ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx[idxPeak]].diff))
                    {
                        if (idxPeak < ptrAdvSlider->peakNum)
                        {
                            for (idxPeak1 = (idxPeak + 1u); idxPeak1 < ptrAdvSlider->peakNum; idxPeak1++)
                            {
                                if (snsIndex == ptrAdvSlider->peakIdx[idxPeak1])
                                {
                                    ptrAdvSlider->peakIdx[idxPeak1] = 0xFFFFu;
                                }
                            }
    ...
    for (idxPeak = 0; idxPeak < ptrAdvSlider->peakNum; idxPeak++)
    {
        if (ptrAdvSlider->peakIdx[idxPeak] != 0xFFFFu)
        {
            ptrAdvSlider->peakIdx1[peakNumLocal] = ptrAdvSlider->peakIdx[idxPeak];
            peakNumLocal++;
        }
    }
    ptrAdvSlider->peakNum = peakNumLocal;

    /* Keep only the strongest peak (single-touch) */
    if (peakNumLocal > 1u)
    {
        strongestPeakDiff = ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx1[0u]].diff;

        for (idxPeak = 1u; idxPeak < peakNumLocal; idxPeak++)
        {
            if (ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx1[idxPeak]].diff > strongestPeakDiff)
            {
                strongestPeakDiff = ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx1[idxPeak]].diff;
                strongestPeakIdx = (uint8_t)idxPeak;
            }
        }

        ptrAdvSlider->peakIdx1[0u] = ptrAdvSlider->peakIdx1[strongestPeakIdx];
        ptrAdvSlider->peakNum = 1u;
    }

    /* Determine touch zone. */
    for (idxPeak = 0; idxPeak < ptrAdvSlider->peakNum; idxPeak++)
    {
        ptrAdvSlider->firstZoneIdx[idxPeak] = ptrAdvSlider->peakIdx1[idxPeak];
        ptrAdvSlider->lastZoneIdx[idxPeak] = ptrAdvSlider->peakIdx1[idxPeak];
    }

    for (idxPeak = 0; idxPeak < ptrAdvSlider->peakNum; idxPeak++)
    {
        /* Determine the left part of touch zone. */
        if (ptrAdvSlider->peakIdx1[idxPeak] > 0u)
        {
            for (snsIndex = ptrAdvSlider->peakIdx1[idxPeak]; snsIndex-- != 0u;)
            {
                if ((256u * ptrWdConfig->ptrSnsContext[snsIndex].diff) > (ptrWdConfig->ptrWdContext->fingerSigThMult * ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx1[idxPeak]].diff))
                {
                    /* Check if this intersection doesn't belong to another touch zone. */
                    temp = 0u;
                    for (idxPeak1 = ptrAdvSlider->peakNum; idxPeak1-- != 0u;)
                    {
                        if ((idxPeak1 != idxPeak))
                        {
                            if ((ptrAdvSlider->firstZoneIdx[idxPeak1] <= snsIndex) && (snsIndex <= ptrAdvSlider->lastZoneIdx[idxPeak1]))
                            {
                                temp++;
                                break;
                            }
                    ...
                    if (temp == 0u)
                    {
                        ptrAdvSlider->firstZoneIdx[idxPeak] = snsIndex;
                    }
                }
                else
                {
        ...
        /* Determine the right part of touch zone. */
        if (ptrAdvSlider->peakIdx1[idxPeak] < (snsCount - 1u))
        {
            for (snsIndex = (ptrAdvSlider->peakIdx1[idxPeak] + 1u); snsIndex < snsCount; snsIndex++)
            {
                if ((256u * ptrWdConfig->ptrSnsContext[snsIndex].diff) > (ptrWdConfig->ptrWdContext->fingerSigThMult * ptrWdConfig->ptrSnsContext[ptrAdvSlider->peakIdx1[idxPeak]].diff))
                {
                    /* Check if this intersection doesn't belong to another touch zone. */
                    ...
                    if (temp == 0u)
                    {
                        ptrAdvSlider->lastZoneIdx[idxPeak] = snsIndex;
                    }
                }
                else
                {
...
/*******************************************************************************
* Function Name: Cy_Capsense_SliderCalculatePositionNormalFinger
****************************************************************************//**
*
* This function calculates position of the detected normal-finger touch.
*
* \param idxPeak
* Specifies the peak index.
*
* \param ptrAdvSlider 
* The pointer to the advanced slider structure.
*
...
{
    uint16_t snsIndex;
    uint16_t maxIndex = ptrAdvSlider->peakIdx1[idxPeak];
    uint16_t maxDiff = ptrWdConfig->ptrSnsContext[maxIndex].diff;
    uint16_t snsCount = ptrWdConfig->numSns;

    uint16_t res = ptrWdConfig->xResolution;
    int32_t centroid[CY_CAPSENSE_INPUT_ARRAY_SIZE] = {0, 0, 0, 0, 0};
    int16_t weightCoeff[CY_CAPSENSE_INPUT_ARRAY_SIZE_HALF + 1] = {0, 256, 512};
    uint16_t calcThreshold = ptrWdConfig->ptrWdContext->calcTh;
    uint16_t offsetOuterEdge = ptrWdConfig->ptrWdContext->offsetOuterEdge;
    uint16_t offsetInnerEdge = ptrWdConfig->ptrWdContext->offsetInnerEdge;
    int32_t denominator;
    int32_t numerator;
    int32_t pos;
    int64_t snsNumTerm;
    int64_t numeratorTerm;

    /* Recalculate peak index as index of the largest sensor in the calculated touch zone. */
    maxDiff = ptrWdConfig->ptrSnsContext[maxIndex].diff;
    for (snsIndex = ptrAdvSlider->firstZoneIdx[idxPeak]; snsIndex <= ptrAdvSlider->lastZoneIdx[idxPeak]; snsIndex++)
    {
        if (ptrWdConfig->ptrSnsContext[snsIndex].diff > maxDiff)
        {
            maxDiff = ptrWdConfig->ptrSnsContext[snsIndex].diff;
            ptrAdvSlider->peakIdx1[idxPeak] = snsIndex;
        }
    }
    maxIndex = ptrAdvSlider->peakIdx1[idxPeak];

    /* Select 5-sensors grid. */
    if (((maxIndex > 1u) && ((ptrWdConfig->ptrSnsContext[maxIndex - 2u].diff) > calcThreshold)))
    {
        centroid[0u] = ((int32_t)ptrWdConfig->ptrSnsContext[maxIndex - 2u].diff) - ((int32_t)calcThreshold);
    }
    if (((maxIndex > 0u) && ((ptrWdConfig->ptrSnsContext[maxIndex - 1u].diff) > calcThreshold)))
    {
        centroid[1u] = ((int32_t)ptrWdConfig->ptrSnsContext[maxIndex - 1u].diff) - ((int32_t)calcThreshold);
    }

    /* centroid[2] is used as devisor in next code and this change to avoid from division on zero exception */
    centroid[2u] = 1;
    if ((maxDiff > calcThreshold))
    {
        centroid[2u] = ((int32_t)maxDiff) - ((int32_t)calcThreshold);
    }
    if (((maxIndex < (snsCount - 1u)) && ((ptrWdConfig->ptrSnsContext[maxIndex + 1u].diff) > calcThreshold)))
    {
        centroid[3u] = ((int32_t)ptrWdConfig->ptrSnsContext[maxIndex + 1u].diff) - ((int32_t)calcThreshold);
    }
    if (((maxIndex < (snsCount - 2u)) && ((ptrWdConfig->ptrSnsContext[maxIndex + 2u].diff) > calcThreshold)))
    {
        centroid[4u] = ((int32_t)ptrWdConfig->ptrSnsContext[maxIndex + 2u].diff) - ((int32_t)calcThreshold);
    }

    /* Set centroid coefficients for outer and inner edge regions of the linear slider. */
    if (((maxIndex == 0u) || (maxIndex == (snsCount - 1u))))
    {
        if (maxIndex == 0u)
        {
            weightCoeff[0u] -= (int16_t)offsetOuterEdge;
        }
        else
        {
            weightCoeff[0u] += (int16_t)offsetOuterEdge;
        ...
        weightCoeff[1u] += (int16_t)offsetOuterEdge;
        weightCoeff[2u] -= ((int16_t)2 * (int16_t)offsetOuterEdge);
    }
    else if (((maxIndex == 1u) || (maxIndex == (snsCount - 2u))))
    {
        weightCoeff[2u] -= (int16_t)offsetInnerEdge;
    }
    else
    {
        /* Do nothing */
    ...
    denominator = centroid[0u] + centroid[1u] + centroid[2u] + centroid[3u] + centroid[4u];
    numerator = (((int32_t)weightCoeff[0u] * centroid[2u]) +
                 (((int32_t)weightCoeff[1u] * (centroid[3u] - centroid[1u])) +
                  ((int32_t)weightCoeff[2u] * (centroid[4u] - centroid[0u]))));
    if (0u != (ptrWdConfig->centroidConfig & CY_CAPSENSE_CALC_METHOD_MASK))
    {
        /* SNS_NUM */

        pos = ((int32_t)128 * (int32_t)snsCount) * denominator;
        snsNumTerm = (((int64_t)128 * (int64_t)denominator) * ((2LL * (int64_t)maxIndex) + 1LL));
        numeratorTerm = (snsNumTerm + (int64_t)numerator);
        pos = (int32_t)((((int64_t)res * numeratorTerm) + (int64_t)pos) / (2LL * (int64_t)pos));
    }
    else
    {
        /* SNS_NUM - 1 */

        pos = ((int32_t)128 * (((int32_t)snsCount) - 1)) * denominator;
        snsNumTerm = (((int64_t)256 * (int64_t)maxIndex) * (int64_t)denominator);
        numeratorTerm = (snsNumTerm + (int64_t)numerator);
        pos = (int32_t)((((int64_t)res * numeratorTerm) + (int64_t)pos) / (2LL * (int64_t)pos));
    }
...
/*******************************************************************************
* Function Name: Cy_Capsense_SliderCalculatePositionFatFinger
****************************************************************************//**
*
* This function calculates position for fat finger based on signals from whole touch zone.
*
* \param idxPeak
* Specifies the peak index.
*
                ...
                const cy_stc_capsense_widget_config_t * ptrWdConfig)
{
    ...
    uint32_t res = ptrWdConfig->xResolution;
    uint32_t pos;

    for (snsIndex = ptrAdvSlider->firstZoneIdx[idxPeak]; snsIndex <= ptrAdvSlider->lastZoneIdx[idxPeak]; snsIndex++)
    {
        denominator += ptrWdConfig->ptrSnsContext[snsIndex].diff;
        numerator += ((uint32_t)ptrWdConfig->ptrSnsContext[snsIndex].diff * (uint32_t)snsIndex);
    }

    pos = ((uint32_t)ptrWdConfig->numSns) * denominator;
    if (pos > 0u)
    {
        pos = ((res * (denominator + (2u * numerator))) + pos) / (2u * pos);
    }
    else
    {
        pos = 0u;
...
#endif /* (CY_CAPSENSE_SLIDER_CENTROID_5X_EN) */
