                ...
                const cy_stc_capsense_context_t * context)
{
    ...
    #endif /* CY_CAPSENSE_PLATFORM_BLOCK_FIFTH_GEN_LP */
        {
            capStatus = (uint32_t)context->ptrWdContext[widgetId].status & (uint32_t)CY_CAPSENSE_WD_ACTIVE_MASK;
        }
    }
    return capStatus;
}
...
/*******************************************************************************
* Function Name: Cy_CapSense_GetLiquidWidgetPosition
****************************************************************************//**
*
* Reports the current liquid level position of the specified liquid level widget.
*
* This function does not process widget data but extracts previously processed
* results from the \ref group_capsense_structures.
*
* \note
* This function is available only for the fifth-generation low power CAPSENSE&trade;
* and is applicable to widgets of the \ref CY_CAPSENSE_WD_LIQUID_LEVEL_E type.
*
* \param widgetId
* Specifies the ID number of the widget. A macro for the widget ID can be found
* in the cycfg_capsense.h file defined as CY_CAPSENSE_<WIDGET_NAME>_WDGT_ID.
...
*
* \return
* Returns the current liquid level position scaled to the widget resolution.
* Returns \ref CY_CAPSENSE_LLS_INVALID if the specified widget is not
* a liquid level widget or a wrong widgetId is specified.
*
*******************************************************************************/
uint16_t Cy_CapSense_GetLiquidWidgetPosition(
                uint32_t widgetId,
                const cy_stc_capsense_context_t * context)
{
    uint16_t position = CY_CAPSENSE_LLS_INVALID;

    if (widgetId < CY_CAPSENSE_TOTAL_WIDGET_COUNT)
    {
        if ((uint8_t)CY_CAPSENSE_WD_LIQUID_LEVEL_E == context->ptrWdConfig[widgetId].wdType)
        {
            position = context->ptrWdConfig[widgetId].ptrWdContext->wdTouch.ptrPosition->x;
        }
    }
...
/*******************************************************************************
* Function Name: Cy_CapSense_GetLiquidPresenceLevel
****************************************************************************//**
*
* Reports the current liquid presence level of the specified
* liquid presence widget.
*
* This function does not process widget data but extracts previously processed
* results from the \ref group_capsense_structures.
*
* \note
* This function is available only for the fifth-generation low power CAPSENSE&trade;
* and is applicable to widgets of the \ref CY_CAPSENSE_WD_LIQUID_PRESENCE_E
* and \ref CY_CAPSENSE_WD_LIQUID_LEVEL_E types when multi-level detection
* feature is enabled.
*
* \param widgetId
* Specifies the ID number of the widget. A macro for the widget ID can be found
...
*
* \return
* Returns the highest confirmed liquid presence level:
* - \ref CY_CAPSENSE_LIQUID_PRESENCE_LEVEL_NONE - No liquid presence detected.
* - \ref CY_CAPSENSE_LIQUID_PRESENCE_LEVEL1     - Liquid presence level 1 detected.
* - \ref CY_CAPSENSE_LIQUID_PRESENCE_LEVEL2     - Liquid presence level 2 detected.
* - \ref CY_CAPSENSE_LIQUID_PRESENCE_LEVEL3     - Liquid presence level 3 detected.
* - \ref CY_CAPSENSE_LLS_INVALID                - The specified widget is not
*   a liquid presence / liquid level widget or a wrong widgetId is specified.
*
*******************************************************************************/
uint32_t Cy_CapSense_GetLiquidPresenceLevel(
                uint32_t widgetId,
                const cy_stc_capsense_context_t * context)
{
    uint32_t presenceLevel = CY_CAPSENSE_LLS_INVALID;
    uint32_t status;

    if (widgetId < CY_CAPSENSE_TOTAL_WIDGET_COUNT)
    {
        if (((uint8_t)CY_CAPSENSE_WD_LIQUID_PRESENCE_E == context->ptrWdConfig[widgetId].wdType) ||
            ((uint8_t)CY_CAPSENSE_WD_LIQUID_LEVEL_E == context->ptrWdConfig[widgetId].wdType))
        {
            status = context->ptrWdContext[widgetId].status;

            if (0u != (status & CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL3_MASK))
            {
                presenceLevel = CY_CAPSENSE_LIQUID_PRESENCE_LEVEL3;
            }
            else if (0u != (status & CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL2_MASK))
            {
                presenceLevel = CY_CAPSENSE_LIQUID_PRESENCE_LEVEL2;
            }
            else if (0u != (status & CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL1_MASK))
            {
                presenceLevel = CY_CAPSENSE_LIQUID_PRESENCE_LEVEL1;
            }
            else
            {
                presenceLevel = CY_CAPSENSE_LIQUID_PRESENCE_LEVEL_NONE;
            }
        }
    }
...
#if (CY_CAPSENSE_LIQUID_LEVEL_FOAM_DETECTION_EN)
/*******************************************************************************
* Function Name: Cy_CapSense_GetFoamWidgetPosition
****************************************************************************//**
*
* Reports the current position of the specified foam widget.
*
* The foam widget is a liquid level sub-widget dedicated to Foam Detection
* measurement. It is placed as the widget immediately preceding the main
* liquid level widget in the widget array. This function does not process
* widget data but extracts previously processed results from the
* \ref group_capsense_structures.
*
* \note
* This function is available only for the fifth-generation low power CAPSENSE&trade;
* when the Liquid Level Foam Detection feature is enabled, and is applicable
* to widgets of the \ref CY_CAPSENSE_WD_LIQUID_LEVEL_E type.
*
* \param widgetId
* Specifies the ID number of the foam widget. A macro for the widget ID can
* be found in the cycfg_capsense.h file defined as
* CY_CAPSENSE_<WIDGET_NAME>_WDGT_ID.
*
* \param context
* The pointer to the CAPSENSE&trade; context structure \ref cy_stc_capsense_context_t.
*
* \return
* Returns the current foam widget position scaled to the widget resolution.
* Returns \ref CY_CAPSENSE_LLS_INVALID if the specified widget is not
* a liquid level widget or a wrong widgetId is specified.
*
*******************************************************************************/
uint16_t Cy_CapSense_GetFoamWidgetPosition(
                uint32_t widgetId,
                const cy_stc_capsense_context_t * context)
{
    uint16_t position = CY_CAPSENSE_LLS_INVALID;
...
#endif /* (CY_CAPSENSE_LIQUID_LEVEL_FOAM_DETECTION_EN) */
#endif /* (CY_CAPSENSE_LIQUID_LEVEL_EN) */
                ...
                cy_stc_capsense_context_t * context)
{
        ...
        (CY_CAPSENSE_WBX_GROUP == ptrWdCfg->senseMethod))
    {
        ...
        crcDataVal.hysteresisVal = ptrWdCxt->hysteresis;
            ...
            crcDataVal.liquidPresenceLevel2Val = ptrWdCxt->liquidPresenceLevel2;
            crcDataVal.liquidPresenceLevel3Val = ptrWdCxt->liquidPresenceLevel3;
        #endif
    }
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    if (widgetId < CY_CAPSENSE_TOTAL_WIDGET_COUNT)
    {
        capStatus = CY_CAPSENSE_STATUS_SUCCESS;
        #if (CY_CAPSENSE_PLATFORM_BLOCK_FOURTH_GEN)
            context->ptrWdContext[widgetId].status &= ~mode;
            context->ptrWdContext[widgetId].status |= (mask & mode);
            ...
            /* Update scan flags for MSC platforms */
                ...
                if (0u != (ptrWdCfg->mfsConfig & CY_CAPSENSE_MFS_EN_MASK))
                {
                    ...
                    context->ptrWdContext[wdIndex].status &= ~mode;
                    context->ptrWdContext[wdIndex].status |= (mask & mode);
                    context->ptrWdContext[wdIndex + CY_CAPSENSE_MFS_CH1_INDEX].status &= ~mode;
                    context->ptrWdContext[wdIndex + CY_CAPSENSE_MFS_CH1_INDEX].status |= (mask & mode);
                    context->ptrWdContext[wdIndex + CY_CAPSENSE_MFS_CH2_INDEX].status &= ~mode;
                    context->ptrWdContext[wdIndex + CY_CAPSENSE_MFS_CH2_INDEX].status |= (mask & mode);
                    #if (CY_CAPSENSE_PLATFORM_BLOCK_FIFTH_GEN_LP)
...
* \note
* This function is available only for the fifth-generation low power CAPSENSE&trade;.
* It is applicable for Liquid Level and Liquid Presence widgets when the tank
* removal detection feature is enabled.
*
* \param widgetId
* Specifies the ID number of the widget. A macro for the widget ID can be found
* in the cycfg_capsense.h file defined as CY_CAPSENSE_<WIDGET_NAME>_WDGT_ID.
...
*
*******************************************************************************/
uint8_t Cy_CapSense_IsTankRemoved(uint32_t widgetId, cy_stc_capsense_context_t * context)
{
    return (0u != (context->ptrWdContext[widgetId].status & CY_CAPSENSE_WD_TANK_REMOVAL_DETECTION_MASK)) ? 1u : 0u;
}
#endif


/*******************************************************************************
* Function Name: Cy_Capsense_GetFingerType
****************************************************************************//**
*
* Returns the detected finger/object type for the specified linear slider 
* widget based on the Small/Large Slider classification algorithm.
*
* \param context
* The pointer to the CAPSENSE&trade; context structure \ref cy_stc_capsense_context_t
...
*
* \return
* Returns the detected finger/object type:
* - CY_CAPSENSE_FINGER_TYPE_NONE         - Finger type has not detected.
* - CY_CAPSENSE_FINGER_TYPE_NORMAL       - Normal finger type. 
* - CY_CAPSENSE_FINGER_TYPE_FAT          - Fat finger type.
* - CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT - Large finger type.
*
*******************************************************************************/
uint8_t Cy_Capsense_GetFingerType(cy_stc_capsense_context_t * context, uint32_t widgetId)
{
    uint8_t fingerType = CY_CAPSENSE_FINGER_TYPE_NONE;

    if ((uint8_t)CY_CAPSENSE_WD_LINEAR_SLIDER_E == context->ptrWdConfig[widgetId].wdType)
    {
        fingerType = (uint8_t)((context->ptrWdContext[widgetId].status & CY_CAPSENSE_WD_FINGER_TYPE_MASK) >> CY_CAPSENSE_WD_FINGER_TYPE_POS);
    }
