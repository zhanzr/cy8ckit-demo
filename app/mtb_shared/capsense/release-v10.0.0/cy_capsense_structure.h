...
typedef enum
{
    ...
    CY_CAPSENSE_TU_CMD_SET_DUTY_CYCLE_AND_SCAN_E = 11u,         /**< Sets duty cycle and executes one scan (used for Foam Detection LLW only) */
...
typedef struct
{
    ...
    uint16_t sigPFC;                                            /**< The 75% of signal per the user-defined touch sensitivity parameter
                                                                   * * For Foam Detection widget defines internal scan parameter */
    #if (CY_CAPSENSE_PLATFORM_BLOCK_FOURTH_GEN)
    ...
    uint16_t fingerTh;                                          /**< Widget Finger Threshold or Liquid Presence Level 1 for Liquid Presence widget */
    #if (CY_CAPSENSE_ENABLE == CY_CAPSENSE_LIQUID_LEVEL_MULTI_LEVEL_EN)
        uint16_t liquidPresenceLevel2;                          /**< Liquid Presence Level 2 for Liquid Presence widget */
        uint16_t liquidPresenceLevel3;                          /**< Liquid Presence Level 3 for Liquid Presence widget */
    #endif
    ...
    uint32_t status;                                             /**< Contains masks:
                                                                   ...
                                                                   * * bit[8] - Liquid Presence Level 1 mask (CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL1_MASK)
                                                                   * * bit[9] - Liquid Presence Level 2 mask (CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL2_MASK)
                                                                   * * bit[10] - Liquid Presence Level 3 mask (CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL3_MASK)
                                                                   * * bit[11..13] - Finger/object type mask (CY_CAPSENSE_WD_FINGER_TYPE_MASK)
                                                                   *   * 0 (CY_CAPSENSE_FINGER_TYPE_NONE)         - Finger type has not detected
                                                                   *   * 1 (CY_CAPSENSE_FINGER_TYPE_NORMAL)       - Normal finger type
                                                                   *   * 2 (CY_CAPSENSE_FINGER_TYPE_FAT)          - Fat finger type
                                                                   *   * 3 (CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT) - Large object type
                                                                */
    cy_stc_capsense_touch_t wdTouch;                            /**< Widget touch structure used for Matrix Buttons, Sliders, and Touchpads */

    #if (CY_CAPSENSE_LINEAR_SLIDER_EN)         
        uint16_t calcTh;                                        /**< Value subtracted from each Diff count in the 5-sensor window before centroid calculation for normal fingers. 
                                                                 * Suppresses small/noisy signals so that only meaningful signal contributes to the position calculation. */
        uint16_t offsetOuterEdge;                               /**< Offset applied to the centroid weights when the peak is at the outermost sensors (index 0 or N–1). 
                                                                 * Used to improve position linearity and accuracy at the physical ends of the slider. */
        uint16_t offsetInnerEdge;                              /**< Offset applied to the centroid weights when the peak is at inner-edge sensors (index 1 or N–2). 
                                                                 * Used to improve position linearity and ensure a smooth transition between edge and core regions of the slider. */
        uint8_t fingerSigThMult;                                /**< Fraction (in 1/256 units) of the local peak signal used as a threshold for including neighboring sensors in a touch zone. 
                                                                 * Higher values make the touch zone narrower; lower values make it wider. */
        uint8_t minFatFingerSize;                               /**< Minimum number of contiguous active sensors in a touch zone required to classify the touch as a fat finger. */
        uint8_t minLargeObjectSize;                             /**< Minimum number of contiguous active sensors in a touch zone required to classify the touch as a large object (palm). */  
    #endif
    ...
    #if (CY_CAPSENSE_RC_ALP_FILTER_EN)
        uint16_t alpOnTh;                                       /**< ALP filter ON threshold */
        uint16_t alpOffTh;                                      /**< ALP filter OFF threshold */
        uint8_t alpFilterCoeff;                                 /**< ALP filter filtering coefficient */
        uint8_t alpMode;                                        /**< ALP filter mode */
    #endif
...
typedef struct
{
    ...
    uint32_t centroidConfig;                                    /**< Bit mask defining centroid configuration including centroid type, number of touches, 
                                                                   * diplexing, Foam Detection, edge correction, etc. */
    ...
    uint32_t rawFilterConfig;                                   /**< Raw count filters configuration */
    ...
    uint16_t cmfThreshold;                                      /**< CMF threshold */
        ...
        uint16_t foamCoefficient;                               /**< Foam Detection coefficient. This decides the scanning cycle for foam sensing mechanism.
                                                                 * \note This field is available for the fifth-generation low power CAPSENSE&trade;.
                                                                 */
    ...
    #if (CY_CAPSENSE_LINEAR_SLIDER_EN)
        uint8_t minFatFingerSizeHyst;                           /**< Hysteresis applied to the minimum fat-finger width. 
                                                                 * Helps prevent frequent switching between normal finger and fat finger 
                                                                 * when the touch width fluctuates around the fat-finger threshold. */
        uint8_t minLargeObjSizeHyst;                            /**< Hysteresis applied to the minimum large-object width. */  
    #endif  
    ...
    #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
        uint8_t cRefSource;                                     /**< Reference capacitor source selection:
                                                                 * * 0 (CY_CAPSENSE_CREF_SOURCE_INTERNAL) - Use internal CDAC (default)
                                                                 * * 1 (CY_CAPSENSE_CREF_SOURCE_EXTERNAL) - Use external capacitor
                                                                 * \note This field is available only for the fifth-generation
                                                                 * CAPSENSE&trade; PSOC&trade; 4 HVMS devices.
                                                                 */
    #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
...
typedef struct
{
    ...
    uint8_t bslnCoeffVal;                                       /**< The value of the .bslnCoeff field of the \ref cy_stc_capsense_widget_context_t structure */
        ...
        uint16_t liquidPresenceLevel2Val;                           /**< The value of the .liquidPresenceLevel2 field of the \ref cy_stc_capsense_widget_context_t structure */
        uint16_t liquidPresenceLevel3Val;                           /**< The value of the .liquidPresenceLevel3 field of the \ref cy_stc_capsense_widget_context_t structure */
    #endif /* (CY_CAPSENSE_LIQUID_LEVEL_MULTI_LEVEL_EN) */
...
typedef struct
{
    ...
    #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
        const cy_stc_capsense_pin_config_t * ptrExtCrefPinConfig;  /**< Pointer to the External Cref pin configuration array.
                                                                    * \note This field is available only for the fifth-generation
                                                                    * CAPSENSE&trade; PSOC&trade; 4 HVMS devices.
                                                                    */
    #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
