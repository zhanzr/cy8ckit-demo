...
/* Helper macros for BIST swSelCswFunc template entries that depend on ext cref configuration */
#if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_DISABLE)
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC4_INDEX    CY_CAPSENSE_CTRLMUX_PIN_STATE_TX_NEGATIVE
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC4_VALUE    (0x00uL)
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC7_INDEX    CY_CAPSENSE_CTRLMUX_PIN_STATE_VDDA2
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC7_VALUE    (CY_CAPSENSE_SM_REG_SW_SEL_CSW6_VDDA2_VALUE)
#else
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC4_INDEX    CY_CAPSENSE_CTRLMUX_PIN_STATE_EXT_CREF0
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC4_VALUE    (CY_CAPSENSE_EXT_CREF0_SW_SEL_CSW_VALUE)
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC7_INDEX    CY_CAPSENSE_CTRLMUX_PIN_STATE_EXT_CREF1
    #define CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC7_VALUE    (CY_CAPSENSE_EXT_CREF1_SW_SEL_CSW_VALUE)
#endif
...
#define CY_CAPSENSE_BIST_SENSING_METHOD_BASE_TEMPLATE   \
    {                                                   \
        ...
        { \
            ...
            [CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC4_INDEX] = CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC4_VALUE, \
            ...
            [CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC7_INDEX] = CY_CAPSENSE_BIST_TEMPLATE_SW_SEL_CSW_FUNC7_VALUE, \
        }, \
        .mode = \
        { \
...
* \note
* This function is available only for the fifth-generation CAPSENSE&trade;.
* Widgets configured with External Cref are excluded from the Cp measurement
* and the CY_CAPSENSE_BIST_BAD_PARAM_E result is returned for such widgets.
*
* \param widgetId
* Specifies the ID number of the widget.
* A macro for the widget ID can be found in the
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    if (NULL != context)
    {
        if ((uint32_t)context->ptrCommonConfig->numWd > widgetId)
        {
                ...
                /* Skip Cp measurement for widgets using external Cref — not supported */
                if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL != context->ptrWdConfig[widgetId].cRefSource)
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    for (slotId = 0u; slotId < CY_CAPSENSE_SLOT_COUNT; slotId++)
    {
        #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
        {
            uint32_t wdIdx = context->ptrScanSlots[slotId].wdId;
            /* Skip slots containing widgets using external Cref */
            if ((CY_CAPSENSE_SLOT_SHIELD_ONLY > wdIdx) &&
                (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == context->ptrWdConfig[wdIdx].cRefSource))
            {
                continue;
            }
        }
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    {
        #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
            /* Skip widgets using external Cref — Cp measurement not supported */
            if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == context->ptrWdConfig[widgetId].cRefSource)
            {
                continue;
            }
...
* \note
* This function is available only for the fifth-generation CAPSENSE&trade;.
* Slots belonging to widgets configured with External Cref are excluded from
* the measurement and the CY_CAPSENSE_BIST_BAD_PARAM_E result is returned
* for such slots.
*
* \param slotId
    ...
    if (NULL != context)
    {
        if (CY_CAPSENSE_SLOT_COUNT > slotId)
        {
            if (CY_CAPSENSE_NOT_BUSY == Cy_CapSense_IsBusy(context))
            {
                ...
                {
                    #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
                        /* Skip Cp measurement for slots containing widgets using external Cref */
                        wdIndex = context->ptrScanSlots[slotId].wdId;
                        if ((CY_CAPSENSE_SLOT_SHIELD_ONLY > wdIndex) &&
                            (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == context->ptrWdConfig[wdIndex].cRefSource))
                        {
                            result = CY_CAPSENSE_BIST_BAD_CONFIG_E;
                        }
                    #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
                ...
                cy_stc_capsense_context_t * context)
{
        ...
        for (i = 0u; i < context->ptrCommonConfig->numPin; i++)
        {
            ...
            for (i = 0u; i < context->ptrCommonConfig->csdShieldNumPin; i++)
            {
                ...
                ptrPinCfg++;
                   ...
                   (CY_CAPSENSE_ENABLE == CY_CAPSENSE_CSD_SHIELD_EN)) */
        #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
            if (NULL != context->ptrExtCrefPinConfig)
            {
                if ((chIndex + context->ptrCommonConfig->channelOffset) ==
                    context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].chId)
                {
                    cswFuncMode |= (uint32_t)(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].padNumber);
                }
                if ((chIndex + context->ptrCommonConfig->channelOffset) ==
                    context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].chId)
                {
                    cswFuncMode |= (uint32_t)(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].padNumber);
                }
            }
        #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
    ...
    /* Generating the common configuration for the coarse initialization and coarse short phase */
        ...
        else 
        {
            ...
            ptrBaseCfg->INIT_CTL3 |= (uint32_t)CY_CAPSENSE_CMOD12_PAIR_SELECTION << MSC_INIT_CTL3_CMOD_SEL_Pos;
        }
    #else
        /* PSOC4-HVMS-128K Devices */
        if (CY_CAPSENSE_CMOD1PADD_PIN == ptrChConfig->pinCmod1)
                ...
                cy_stc_capsense_context_t * context)
{
        ...
        (CY_CAPSENSE_CSX_GROUP == context->ptrActiveScanSns[0u].currentSenseMethod))
    {
        ...
        #if ((CY_CAPSENSE_SENSOR_CONNECTION_MODE == CY_CAPSENSE_CTRLMUX_SENSOR_CONNECTION_METHOD) && \
             (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_DISABLE))
            mode = 1u;
        #endif
    }
    /*
    ...
    ptrSensorCfg[CY_CAPSENSE_SNS_SCAN_CTL_INDEX] |= ((uint32_t)context->ptrBistContext->eltdCapSubConvNum - 1u) << MSC_SNS_SCAN_CTL_NUM_SUB_CONVS_Pos;
        ...
        #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_DISABLE) */
        else
        {
            /* Do nothing */
        }
        ...
        #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
            /* Set inactive ext cref pads to HIGH_Z to disconnect from sense bus */
            if (NULL != context->ptrExtCrefPinConfig)
            {
                padMask = 0u;
                if ((chIndex + context->ptrCommonConfig->channelOffset) ==
                    context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].chId)
                {
                    padMask |= (uint32_t)(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].padNumber);
                }
                if ((chIndex + context->ptrCommonConfig->channelOffset) ==
                    context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].chId)
                {
                    padMask |= (uint32_t)(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].padNumber);
                }
                Cy_CapSense_CalculateMaskRegisters(padMask, CY_CAPSENSE_CTRLMUX_PIN_STATE_HIGH_Z, ptrSensorCfg);
            }
        #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
...
void Cy_CapSense_BistDisableMode(cy_stc_capsense_context_t * context)
{
    ...
    Cy_CapSense_BistSwitchAllExternalCapPinState(CY_CAPSENSE_BIST_IO_STRONG_E, context);
        ...
        {
            Cy_CapSense_SsConfigPinRegisters(
                context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].pcPtr,
                (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].pinNumber,
                CY_CAPSENSE_DM_GPIO_STRONG_IN_OFF, CY_CAPSENSE_HSIOM_SEL_GPIO);
            Cy_GPIO_Clr(context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].pcPtr,
                (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].pinNumber);
            Cy_CapSense_SsConfigPinRegisters(
                context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].pcPtr,
                (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].pinNumber,
                CY_CAPSENSE_DM_GPIO_STRONG_IN_OFF, CY_CAPSENSE_HSIOM_SEL_GPIO);
            Cy_GPIO_Clr(context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].pcPtr,
                (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].pinNumber);
        }
    #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
