                ...
                const cy_stc_capsense_context_t * context)
{
    cy_capsense_status_t result = CY_CAPSENSE_STATUS_SUCCESS;
        ...
        if (*ptrSnsBslnInv != bslnInv)
        {
            ...
            ptrWdContext->status &= (uint32_t)~(uint32_t)CY_CAPSENSE_WD_WORKING_MASK;
        ...
        for (freqChIndex = 0u; freqChIndex < CY_CAPSENSE_CONFIGURED_FREQ_NUM; freqChIndex++)
        {
            for (snsIndex = 0u; snsIndex < ptrWdCfg->numSns; snsIndex++)
            {
                ...
                ptrHistory = &ptrWdCfg->ptrRawFilterHistory[historyOffset];
                    ...
                    if (0u != (CY_CAPSENSE_RC_FILTER_ALP_EN_MASK & rawFilterCfg))
                    {
                        Cy_CapSense_AlpInitialize_Lib((cy_stc_capsense_alp_fltr_channel_t *)ptrHistory, (const uint16_t *)&ptrSnsCtx->raw);
                    }
                    else
                #endif
                {
                ...
                uint8_t * ptrSnsRawHistoryLow)
{
    ...
    {
        tempRaw = (uint32_t)((uint32_t)ptrSnsContext->raw) << CY_CAPSENSE_IIR_SHIFT_PERFORMANCE;
        tempHistory  = (uint32_t)((uint32_t)(*ptrSnsRawHistory)) << CY_CAPSENSE_IIR_SHIFT_PERFORMANCE;
...
void Cy_CapSense_FtRunEnabledFiltersInternal(
                ...
                uint8_t * ptrSnsRawHistoryLow)
{
    uint32_t rawFilterCfg;
    uint16_t * ptrSnsRawHistoryLocal = ptrSnsRawHistory;
    #if (CY_CAPSENSE_RC_ALP_FILTER_EN)
        cy_stc_capsense_alp_fltr_config_t alpFilterConfig;
    ...
    rawFilterCfg = ptrWdConfig->rawFilterConfig;
        ...
        if (0u != (CY_CAPSENSE_RC_FILTER_ALP_EN_MASK & rawFilterCfg))
        {
            alpFilterConfig.configParam0 = ptrWdCxt->alpOnTh;
            alpFilterConfig.configParam1 = ptrWdCxt->alpOffTh;
            alpFilterConfig.configParam2 = ptrWdCxt->proxTh;
            alpFilterConfig.configParam3 = ptrWdCxt->nNoiseTh;
            alpFilterConfig.configParam4 = ptrWdCxt->noiseTh;
            alpFilterConfig.configParam5 = ptrWdCxt->hysteresis;
            alpFilterConfig.configParam6 = ptrWdCxt->alpFilterCoeff;
            alpFilterConfig.configParam7 = ptrWdCxt->alpMode;
            Cy_CapSense_AlpRun_Lib((cy_stc_capsense_alp_fltr_channel_t *)ptrSnsRawHistoryLocal,
                    (const cy_stc_capsense_alp_fltr_config_t *)&alpFilterConfig,
                    &ptrSnsContext->raw, (const uint16_t *)&ptrSnsContext->bsln);
        }
        else
    #endif
    {
