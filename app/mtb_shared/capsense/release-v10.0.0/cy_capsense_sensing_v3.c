...
*
* For widgets configured with External Cref, the Reference CDAC calibration and
* normalization phases are skipped because the internal CDAC reference
* is not used. Only the Compensation CDAC Divider and Compensation CDAC
        ...
        calibStatus = CY_CAPSENSE_STATUS_SUCCESS;
            ...
            /* Finds max RefCDAC and normalizes RefCDAC */
                ...
                for (curWdIndex = 0u; curWdIndex < ptrCommonCfg->numWd; curWdIndex++)
                {
                    ...
                    {
                        #if (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN)
                            /* Skip normalization for ext Cref widgets - RefCDAC was not calibrated */
                            if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL != context->ptrWdConfig[curWdIndex].cRefSource)
                        #endif /* (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN) */
                        {
...
*
* For widgets configured with External Cref, the Reference CDAC calibration phase is
* skipped. Only the Compensation CDAC Divider and Compensation CDAC calibration are
* performed.
*
* \note
* This function is available only for the fifth-generation CAPSENSE&trade;.
            ...
            switch (ptrWdCfg->senseMethod)
            {
                    ...
                    case CY_CAPSENSE_CSD_GROUP:
                        ...
                        /* Calibrate RefCDAC with disabled CompCDAC */
                            ...
                            #if (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN)
                            /* Skip RefCDAC calibration for ext Cref widgets */
                            if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL != ptrWdCfg->cRefSource)
                            #endif /* (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN) */
                            {
                                context->ptrCommonContext->status |= CY_CAPSENSE_MW_STATE_CALIBRATION_SINGLE_MASK;
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    #endif /* ((CY_CAPSENSE_ENABLE == CY_CAPSENSE_CSX_EN) && (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_DISABLE)) */
            ...
            switch (configuration)
            {
                ...
                case CY_CAPSENSE_HW_CONFIG_REGULAR_SCANNING:
                    ...
                    Cy_CapSense_SetIOsInDefaultState(context);
                        ...
                        /* Set ext cref pins HSIOM to CSD_SENSE once during HW configuration.
                         * Per-slot activation is handled by function mode mask registers. */
                ...
                case CY_CAPSENSE_HW_CONFIG_BIST_FUNCTIONALITY:
                    ...
                    #endif
                    #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
                        /* Set ext cref pins HSIOM to CSD_SENSE for BIST mode */
                        if (NULL != context->ptrExtCrefPinConfig)
                        {
                            Cy_CapSense_SsConfigPinRegisters(
                                context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].pcPtr,
...
*
* \note
* Widgets configured with External Cref are excluded from all SmartSense tuning steps.
*
* \param context
* The pointer to the CAPSENSE&trade; context structure \ref cy_stc_capsense_context_t.
*
...
cy_capsense_status_t Cy_CapSense_SsAutoTune(cy_stc_capsense_context_t * context)
{
    ...
    for (i = 0u; i < ptrCommonCfg->numWd; i++)
    {
        ...
        {
            #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
                /* Skip SmartSense for ext Cref widgets */
                if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == ptrWdCfg->cRefSource)
                {
                    ptrWdCfg++;
                    continue;
    ...
    for (i = 0u; i < CY_CAPSENSE_WIDGET_COUNT; i++)
    {
        ...
        {
            #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
                /* Skip SmartSense tuning for ext Cref widgets */
                if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == ptrWdCfg->cRefSource)
                {
                    ptrWdCfg++;
                    continue;
    ...
    /* Step #4: Align SnsClkDiv and Nsub for multi-channel solution */
        ...
        for (i = 0u; i < CY_CAPSENSE_SLOT_COUNT; i++)
        {
            ...
            for (j = 0u; j < CY_CAPSENSE_TOTAL_CH_NUMBER; j++)
            {
                ...
                {
                    #if (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN)
                        /* Skip ext Cref widgets - SmartSense not supported */
                        if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == ptrWdCfg[wdIndex[j]].cRefSource)
                        {
                            continue;
                        }
                    #endif /* (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN) */
    ...
    for (i = 0u; i < CY_CAPSENSE_WIDGET_COUNT; i++)
    {
        ...
        {
            #if (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN)
                /* Skip ext Cref widgets - their params were not modified by SmartSense */
                if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL != ptrWdCfg->cRefSource)
            #endif /* (CY_CAPSENSE_ENABLE == CY_CAPSENSE_EXT_CREF_EN) */
            {
                ptrWdCxt = ptrWdCfg->ptrWdContext;
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    for (slotId = 0u; slotId < CY_CAPSENSE_SLOT_COUNT; slotId++)
    {
        ...
        for (curChIndex = 0u; curChIndex < CY_CAPSENSE_TOTAL_CH_NUMBER; curChIndex++)
        {
            ...
            if (CY_CAPSENSE_CDAC_DITHERING_MODE_AUTO == ptrWdCfg->cdacDitherScaleMode)
            {
                ...
                {
                    #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
                        /* Skip auto-dither scan for ext Cref widgets */
                        if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL != ptrWdCfg->cRefSource)
                    #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
                    {
    ...
    if (status == CY_CAPSENSE_STATUS_SUCCESS)
    {
        for (wdIndex = 0u; wdIndex < CY_CAPSENSE_TOTAL_WIDGET_COUNT; wdIndex++)
        {
            ...
            {
                #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
                    /* Skip auto-dither calculation for ext Cref widgets */
                    if (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == ptrWdCfg->cRefSource)
                    {
                        continue;
                    }
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    scanConfigTmp[CY_CAPSENSE_SNS_CTL_INDEX] |= ((((uint32_t)CY_CAPSENSE_CDAC_DITHERING_KREF - 1u) << MSC_SNS_CTL_SENSE_DIV_Pos) |
    ...
    scanConfigTmp[CY_CAPSENSE_SNS_SCAN_CTL_INDEX] |= ((((uint32_t)CY_CAPSENSE_CDAC_DITHERING_NUM_SUBCONV - 1u) << MSC_SNS_SCAN_CTL_NUM_SUB_CONVS_Pos) |
                                                     (((uint32_t)CY_CAPSENSE_CDAC_DITHERING_COMP_DIV - 1u) << MSC_SNS_SCAN_CTL_COMP_DIV_Pos));

    /* Configures Cap DACs and Clock reference rate*/
    scanConfigTmp[CY_CAPSENSE_SNS_CDAC_CTL_INDEX] = 0u;
    scanConfigTmp[CY_CAPSENSE_SNS_CDAC_CTL_INDEX] |= (((uint32_t)CY_CAPSENSE_CDAC_DITHERING_CDAC_REF << MSC_SNS_CDAC_CTL_SEL_RE_Pos) |
                                                     ((uint32_t)CY_CAPSENSE_CDAC_DITHERING_CLOCK_REF_RATE << MSC_SNS_CDAC_CTL_CLOCK_REF_RATE_Pos));
