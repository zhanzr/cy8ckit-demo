                ...
                const cy_stc_capsense_context_t * context)
{
...
#endif /* CY_CAPSENSE_PLATFORM_BLOCK_FIFTH_GEN_LP */
    {
        ...
        ptrWdCxt->status &= (uint32_t)~(uint32_t)CY_CAPSENSE_WD_ACTIVE_MASK;
        /* Clear sensor status */
                ...
                const cy_stc_capsense_context_t * context)
{
    ...
    #if (CY_CAPSENSE_DISABLE != CY_CAPSENSE_RAWCOUNT_FILTER_EN)
        snsHistorySize = ptrWdCfg->rawFilterConfig & CY_CAPSENSE_RC_FILTER_SNS_HISTORY_SIZE_MASK;
    #endif
        ...
        for (snsIndex = 0u; snsIndex < ptrWdCfg->numSns; snsIndex++)
        {
            ...
            /* Run auto-tuning activities */
                ...
                if ((((uint8_t)CY_CAPSENSE_WD_LOW_POWER_E != ptrWdCfg->wdType) && 
                     ...
                     (0u == (CY_CAPSENSE_RC_FILTER_ALP_EN_MASK & ptrWdCfg->rawFilterConfig)) &&
                ...
                const cy_stc_capsense_context_t * context)
{
    ...
    /* Run auto-tuning activities */
        ...
        if ((((uint8_t)CY_CAPSENSE_WD_LOW_POWER_E != ptrWdConfig->wdType) &&
            ...
            (0u == (CY_CAPSENSE_RC_FILTER_ALP_EN_MASK & ptrWdConfig->rawFilterConfig)) &&
                ...
                const cy_stc_capsense_widget_config_t * ptrWdConfig)
{
    ...
    for (snsIndex = 0u; snsIndex < ptrWdConfig->numSns; snsIndex++)
    {
        ...
        {
            ptrWdCxt->status |= (uint32_t)CY_CAPSENSE_WD_ACTIVE_MASK;
        }
...
*
* For the Linear  Slider, the centroid algorithm classifies the touch
* object type (a normal finger, fat finger, or large object) based on the
* touch zone width. See the Cy_CapSense_DpCentroidLinear() description
* for details.
*
* This function is expected to be called after every new widget scan. If it is
* called multiple times for the same scan data, the debounce algorithm
* will not work properly.
*
* \param ptrWdConfig
                ...
                const cy_stc_capsense_widget_config_t * ptrWdConfig)
{
    ...
    if (0u == wdActive)
    {
        /* Reset debounce counter */
        (*ptrDebounceCnt) = ptrWdCxt->onDebounce;
        ptrWdCxt->status &= (uint32_t)~((uint32_t)CY_CAPSENSE_WD_ACTIVE_MASK | (uint32_t)CY_CAPSENSE_WD_FINGER_TYPE_MASK);
    }
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    #if ((CY_CAPSENSE_LIQUID_LEVEL_MULTI_LEVEL_EN) || (CY_CAPSENSE_LIQUID_LEVEL_TANK_REMOVAL_DETECTION_EN))
        cy_stc_capsense_widget_context_t * ptrWdCxt = ptrWdConfig->ptrWdContext;
    #endif

    #if (CY_CAPSENSE_ENABLE == CY_CAPSENSE_LIQUID_LEVEL_FOAM_DETECTION_EN)
        uint16_t foamCorrection;
    #endif

    #if (CY_CAPSENSE_LIQUID_LEVEL_MULTI_LEVEL_EN)
        uint32_t multiLevels = (ptrWdConfig->centroidConfig & CY_CAPSENSE_LLW_MULTI_LEVEL_MASK) >> CY_CAPSENSE_LLW_MULTI_LEVEL_POS;
        uint32_t liquidLevelDetection = 0u;
        uint32_t liquidLevelDetectionMask = 0u;
        uint32_t lvl;
        uint8_t * ptrDebounceCnt = ptrWdConfig->ptrDebounceArr;
    #endif
    ...
    if (CY_CAPSENSE_LLW_NOT_VALID_DATA != liquidLevel)
    {
            ...
            {  
                ptrWdCxt->status |= (uint32_t)CY_CAPSENSE_WD_TANK_REMOVAL_DETECTION_MASK;
            }
        ...
        #if (CY_CAPSENSE_LIQUID_LEVEL_MULTI_LEVEL_EN)
            for (lvl = 1u; lvl <= multiLevels; lvl++)
            {
                /* Calculate liquid level detection threshold */
                switch(lvl)
                {
                    case 1u:
                        liquidLevelDetection = (0u == (ptrWdCxt->status & CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL1_MASK)) ?
                            ((uint32_t)ptrWdCxt->fingerTh + ptrWdCxt->hysteresis) :
                            ((uint32_t)ptrWdCxt->fingerTh - ptrWdCxt->hysteresis);
                            liquidLevelDetectionMask = (uint32_t)CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL1_MASK;
                        break;
                    case 2u:
                        liquidLevelDetection = (0u == (ptrWdCxt->status & CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL2_MASK)) ?
                            ((uint32_t)ptrWdCxt->liquidPresenceLevel2 + ptrWdCxt->hysteresis) :
                            ((uint32_t)ptrWdCxt->liquidPresenceLevel2 - ptrWdCxt->hysteresis);
                            liquidLevelDetectionMask = (uint32_t)CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL2_MASK;
                        break;
                    case 3u:
                        liquidLevelDetection = (0u == (ptrWdCxt->status & CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL3_MASK)) ?
                            ((uint32_t)ptrWdCxt->liquidPresenceLevel3 + ptrWdCxt->hysteresis) :
                            ((uint32_t)ptrWdCxt->liquidPresenceLevel3 - ptrWdCxt->hysteresis);
                            liquidLevelDetectionMask = (uint32_t)CY_CAPSENSE_WD_LIQUID_PRESENCE_LEVEL3_MASK;
                        break;
                    default:
                        /* Invalid liquid detection level */
                        break;
                }
                ...
                /* No liquid or liquid below threshold */
                if (newTouch.ptrPosition->x < liquidLevelDetection)
                {
                    /* Reset debounce counter */
                    *ptrDebounceCnt = ptrWdCxt->onDebounce;
                    ptrWdCxt->status &= ~liquidLevelDetectionMask;
                }
