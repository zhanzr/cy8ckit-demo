                ...
                cy_stc_capsense_context_t * context)
{
    ...
    for (i = 0u; i < CY_CAPSENSE_CTRLMUX_PIN_STATE_NUMBER; i++)
    {
        ...
        /* Configure SW_SEL_CSW_FUNC entries for External Cref function codes */
        ptrBaseCfg->swSelCswFunc[CY_CAPSENSE_CTRLMUX_PIN_STATE_EXT_CREF0] =
                CY_CAPSENSE_EXT_CREF0_SW_SEL_CSW_VALUE;
        ptrBaseCfg->swSelCswFunc[CY_CAPSENSE_CTRLMUX_PIN_STATE_EXT_CREF1] =
                CY_CAPSENSE_EXT_CREF1_SW_SEL_CSW_VALUE;
    ...
    ptrBaseCfg->initCtl2 &= ~MSC_INIT_CTL2_NUM_INIT_CMOD_34_SHORT_CYCLES_Msk;
        ...
        else 
        {
            ...
            ptrBaseCfg->initCtl3 |= (uint32_t)CY_CAPSENSE_CMOD12_PAIR_SELECTION << MSC_INIT_CTL3_CMOD_SEL_Pos;
        }
    #else
        /* PSOC4-HVMS-128K Devices */
        if (CY_CAPSENSE_CMOD1PADD_PIN == ptrChConfig->pinCmod1)
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    uint32_t * ptrSensorCfgLocal = ptrSensorCfg;
        ...
        #if (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE)
            /* Exclude ext cref pads from inactive mask */
            if (NULL != context->ptrExtCrefPinConfig)
            {
                if ((chId + context->ptrCommonConfig->channelOffset) ==
                    context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].chId)
                {
                    snsMaskInactive &= (uint32_t)~(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].padNumber);
                }
                if ((chId + context->ptrCommonConfig->channelOffset) ==
                    context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].chId)
                {
                    snsMaskInactive &= (uint32_t)~(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].padNumber);
                }
            }
        #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
                ...
                Cy_CapSense_CalculateMaskRegisters(snsMaskInactive, snsFuncState, &sensorCfgTmp[0u]);
                    ...
                    if (NULL != context->ptrExtCrefPinConfig)
                    {
                        ...
                        if ((chId + context->ptrCommonConfig->channelOffset) ==
                            context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].chId)
                        {
                            snsMask |= (uint32_t)(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF0].padNumber);
                        }
                        if ((chId + context->ptrCommonConfig->channelOffset) ==
                            context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].chId)
                        {
                            snsMask |= (uint32_t)(0x01uL << (uint32_t)context->ptrExtCrefPinConfig[CY_CAPSENSE_PIN_IDX_EXT_CREF1].padNumber);
                        }
                        Cy_CapSense_CalculateMaskRegisters(snsMask, CY_CAPSENSE_CTRLMUX_PIN_STATE_HIGH_Z, &sensorCfgTmp[0u]);
                    }
                #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
            }
            else
            {
                ...
                /* EXTERNAL CREF PADS */
                    ...
                    if (NULL != context->ptrExtCrefPinConfig)
                    {
                        ...
                        /* Override with ext cref function codes for ext-cref widget slots */
                        if ((CY_CAPSENSE_SLOT_SHIELD_ONLY > slotValue) &&
                            (CY_CAPSENSE_CREF_SOURCE_EXTERNAL == context->ptrWdConfig[wdIndex].cRefSource))
                        {
                            snsMask = 0u;
                            ...
                            Cy_CapSense_CalculateMaskRegisters(snsMask, CY_CAPSENSE_CTRLMUX_PIN_STATE_EXT_CREF0, &sensorCfgTmp[0u]);
                            ...
                            Cy_CapSense_CalculateMaskRegisters(snsMask, CY_CAPSENSE_CTRLMUX_PIN_STATE_EXT_CREF1, &sensorCfgTmp[0u]);
                        }
                    }
                #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
            }
                ...
                const cy_stc_capsense_context_t * context)
{
    ...
    if (CY_CAPSENSE_ENABLE == ptrWdCfg->ptrWdContext->cdacDitherEn)
    {
        ...
        if (0u == (CY_CAPSENSE_MW_STATE_CALIBRATION_SINGLE_MASK & context->ptrCommonContext->status))
        {
            ...
            {
                /* Zero reference CDAC for External Cref widgets */
                snsCdacCtlReg &= ~MSC_SNS_CDAC_CTL_SEL_RE_Msk;
            }
            else
            #endif /* (CY_CAPSENSE_EXT_CREF_EN == CY_CAPSENSE_ENABLE) */
            {
