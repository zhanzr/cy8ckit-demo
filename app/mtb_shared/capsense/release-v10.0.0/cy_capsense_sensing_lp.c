...
/* LLS Foam Detection duty cycle configuration */
...
static cy_capsense_status_t Cy_CapSense_ShortIntegration(
                ...
                uint32_t numberSlots,
                cy_stc_capsense_context_t * context);
#endif /* (CY_CAPSENSE_ENABLE == CY_CAPSENSE_LIQUID_LEVEL_FOAM_DETECTION_EN) */
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    /* Dummy scan */
        ...
        if (CY_CAPSENSE_CSX_GROUP == context->ptrWdConfig[widgetId].senseMethod)
        {
            ...
            #endif
            calibrationStatus |= Cy_CapSense_ScanSlotInternalCPU(snsFrameType,
                    context->ptrWdConfig[widgetId].firstSlotId, context);
        }
    #endif
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    /* Dummy scan */
        ...
        if (CY_CAPSENSE_CSX_GROUP == ptrWdCfg->senseMethod)
        {
            ...
            #endif
            calibStatus |= Cy_CapSense_ScanSlotInternalCPU(snsFrameType,
                    ptrWdCfg->firstSlotId, context);
        }
    #endif
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    Cy_CapSense_SetupCpuOperatingMode(context);
        ...
        {
            /* Configures Foam Detection scan */
    ...
    for (wdIndex = 0u; wdIndex < CY_CAPSENSE_TOTAL_WIDGET_COUNT; wdIndex++)
    {
        ...
        snsClkSrc = ptrWdCxt->snsClkSource;
                   ...
                   (0u != (ptrWdCfg->centroidConfig & CY_CAPSENSE_LLW_FOAM_EN_MASK)))
            {
                ...
                * foam-detection sub-widget when configured both. 
                */
                snsClkSrc = 0u;
            }
        #endif
...
* like Foam Detection feature can't be scanned with the others in one scan.
*
* \note
* This function is available only for the fifth-generation low power CAPSENSE&trade;.
*
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    ptrWdCfg->ptrWdContext->status &= (uint32_t)~(uint32_t)CY_CAPSENSE_WD_WBX_COMPENSATION_DIRECTION_MASK;
    ...
    ptrSnsCxt->cdacComp = 0u;
    cdacMask = (uint8_t)CY_CAPSENSE_CAL_COMP_CDAC_MIDDLE_CODE;
    ptrWdCfg->ptrWdContext->status |= (uint32_t)CY_CAPSENSE_WD_WBX_COMPENSATION_DIRECTION_MASK;
    while (0u != cdacMask)
    {
        /* Scans a widget with newly defined Ccomp */
        ptrSnsCxt->cdacComp |= cdacMask;
