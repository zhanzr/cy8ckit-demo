...
cy_capsense_status_t Cy_CapSense_Init(cy_stc_capsense_context_t * context)
{
    ...
    if (NULL != context)
    {
        ...
        {
            ptrWdCxt->status |= (uint32_t)((uint32_t)CY_CAPSENSE_WD_ENABLE_MASK | (uint32_t)CY_CAPSENSE_WD_WORKING_MASK);
            ptrWdCxt++;
        }
            ...
            for (wdIndex = 0u; wdIndex < CY_CAPSENSE_TOTAL_WIDGET_COUNT; wdIndex++)
            {
                ...
                {
                    ptrWdCxt->status |= (uint32_t)CY_CAPSENSE_WD_MAXCOUNT_CALC_MASK;
                }
                if (0u == ptrWdCxt->maxRawCountRow)
                {
                    ptrWdCxt->status |= (uint32_t)CY_CAPSENSE_WD_MAXCOUNT_ROW_CALC_MASK;
                }
                ptrWdCxt++;
            }
...
cy_capsense_status_t Cy_CapSense_Enable(cy_stc_capsense_context_t * context)
{
    ...
    if (CY_CAPSENSE_STATUS_SUCCESS == result)
    {
        ...
        /* Scan each widget separately if the MPSC is enabled */
                 ...
                 (CY_CAPSENSE_ENABLE == CY_CAPSENSE_LIQUID_LEVEL_FOAM_DETECTION_EN) || \
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    if (0u == state)
    {
        context->ptrWdContext[widgetId].status &= (uint32_t)(~(uint32_t)CY_CAPSENSE_WD_FACTORY_CALIBRATION_MASK);
    }
    else
    {
        context->ptrWdContext[widgetId].status |= (uint32_t)CY_CAPSENSE_WD_FACTORY_CALIBRATION_MASK;
    }
}
