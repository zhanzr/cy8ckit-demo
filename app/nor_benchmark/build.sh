...
BENCH_SRCS=("bench/utils.c" "smif/smif_nor.c")
OBJS_CM0P=()
OBJS_CM4=()
...
for f in "${BENCH_SRCS[@]}"; do
  base=$(basename "$f" .c)
  "$CC" "${CM0P_CFLAGS[@]}" -c "$f" -o "$BUILD/cm0p_$base.o"
  OBJS_CM0P+=("$BUILD/cm0p_$base.o")
done
...
for f in "${BENCH_SRCS[@]}"; do
  base=$(basename "$f" .c)
  "$CC" "${CM4_CFLAGS[@]}" -c "$f" -o "$BUILD/cm4_$base.o"
  OBJS_CM4+=("$BUILD/cm4_$base.o")
done
