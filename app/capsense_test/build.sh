#!/usr/bin/env bash
# app/capsense_test - CAPSENSE buttons + 5-segment slider demo on
# CY8CKIT-062-BLE (port of Infineon mtb-example-psoc6-capsense-buttons-slider).
#
# Offline ModusToolbox build (deps already fetched into the workspace
# mtb_shared). Produces build/CY8CKIT-062-BLE/Debug/capsense_test.hex.
#
...
"$CY_TOOLS_PATHS/modus-shell/bin/bash.exe" -l -c "
  cd /cygdrive/d/cy8ckit-prj/app/capsense_test &&
  make build \
    SEARCH_TARGET_CY8CKIT-062-BLE='$BSP' \
    SEARCH_core-make='$CORE_MAKE' \
    CY_BASELIB_PATH='$RECIPE'
"
echo "=== built build/CY8CKIT-062-BLE/Debug/capsense_test.hex ==="
